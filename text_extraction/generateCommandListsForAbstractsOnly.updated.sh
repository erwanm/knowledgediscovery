...
find $unfilteredMedlineDir -type f -name '*.xml' | sort | xargs -I FILE echo "$python $cooccurrenceMajigger --binaryTermsFile $wordlist --abstractsFile FILE --outFile $HERE/mined/abstracts/\`basename FILE\`.out > $HERE/logs/abstracts/\`basename FILE\`.out 2>&1" > commands_abstracts.txt
